...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    if (_light.getCurrentPhase() == TrafficLight::TrafficLightPhase::red)
    {
        std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " the light is red." << std::endl;
        _light.waitForGreen();
    }
    if (_light.getCurrentPhase() == TrafficLight::TrafficLightPhase::green)
    {
        std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->getID() << " the light is green." << std::endl;
    }
    lck.unlock();
}
...
{
    // FP.6a : In Intersection.h, add a private member _light of type TrafficLight. At this position, start the simulation of _light.
    _light.simulate();
